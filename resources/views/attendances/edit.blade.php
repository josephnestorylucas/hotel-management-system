            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="event_pass_id" id="event_pass_id" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        ...
                        @endphp
                        <option value="{{ $pass->id }}" data-tier="{{ $pass->tier_type }}" {{ old('event_pass_id', $attendance->event_pass_id) == $pass->id ? 'selected' : '' }}>{{ ucfirst($pass->tier_type) }} - {{ $accessLabel }}</option>
                        @endforeach
                    </select>
                    <input type="hidden" name="pass_type" id="pass_type_input" value="{{ old('pass_type', $attendance->pass_type) }}">
                </div>
                <div>
